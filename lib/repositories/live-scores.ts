...
export async function syncLiveScores() {
  ...
  if (!supabase) {
    return { ok: false as const, reason: "Supabase no configurado.", httpStatus: 500 };
  }
  ...
  if (windowedMatchesQuery.error || !windowedMatchesQuery.data) {
    return { ok: false as const, reason: "No se pudieron cargar matches.", httpStatus: 500 };
  }

  const windowedMatches = windowedMatchesQuery.data;
  if (windowedMatches.length === 0) {
    return { ok: true as const, source: "espn", inWindow: 0, actuallyUpdated: 0, applied: [] as unknown[] };
  }

  const teamsQuery = await supabase.from("teams").select("id, fifa_code");
  if (teamsQuery.error || !teamsQuery.data) {
    return { ok: false as const, reason: "No se pudieron cargar teams.", httpStatus: 500 };
  }
  ...
  if (!espnResponse.ok) {
    return { ok: false as const, reason: `ESPN ${espnResponse.status}`, httpStatus: 502 };
  }
